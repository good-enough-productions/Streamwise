...
### The 4 Main Tabs
1. **Watchlist (Queue):** Your unified queue across movies, TV series, and podcast recommendations. Features **First-Run Onboarding Wizard** (interactive 3-step setup), **Persistent "Free to Me" default**, **Format & Status Filtering** (`All`, `🎬 Movies`, `📺 TV Shows`, `🎙️ Podcast Recs`, `🗓️ Release Radar`), **1-Tap Letterboxd Import** (zero-API-key public profile crawler & CSV parser), **Instant Undo** (recovers accidentally deleted movies via animated banner), **Duration Chips** (`< 90m`, `< 120m`) and **Duration Sorting** ("Shortest Duration") powered by exact TMDB movie runtime metadata, live matching filter counts, compact `MediaItemCard` layouts with expandable synopses, and Olivia's synthesized cultural trivia.
